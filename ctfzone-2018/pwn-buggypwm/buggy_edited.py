...
class CARM:
    ...
    def __command_syscall(self,amplifier):
        if amplifier==1:
            ...
            print("DEBUG: Expecting %d values" % self.edx.real.value)
            s=delete_not_printable(raw_input())
    ...
    def processComand(self):
        ...
        print(self.eip)
        cmd_name = self.commands[opcode].__name__ if opcode in self.commands else "<unk>"
        print(hex(opcode), amplifier, cmd_name)
        print(self.eax)
        print(self.ebx)
        print(self.ecx)
        print(self.edx)
